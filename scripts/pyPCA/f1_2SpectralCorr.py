...
def main():
    ...
    parser.add_argument('site_a', type=int, help="First site to correlate with")
    parser.add_argument('site_b', type=int, help="Second site to correlate with")
    ...
    num_t = 0
    size_Ct_iab = (0,0,0,0) # Set value with data from E_tij

    with h5py.File(h5_filename, 'r+') as f:
        E_tij = f[h5time]
        num_t = int(np.round(args.lenCt / E_tij.attrs['dt']))
        size_Ct_iab = (num_t, E_tij.shape[1], E_tij.shape[2], E_tij.shape[2])

        disk_size = np.product(size_Ct_iab) * 8 / float(10**9)
        print "Note: Estimated maximum size on disk: {0:.1f} GB".format(disk_size)
        print "Note: Estimated maximum size on disk per site: {0:.1f} GB".format(disk_size / float(size_Ct_iab[1]))
        ...
        if args.newCt and h5ct in f:
            ...
            f.create_dataset(h5ct, size_Ct_iab)
